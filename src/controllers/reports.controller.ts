...
export const generateUserReport = asyncHandler(async (req: Request, res: Response) => {
  ...
  const userSummary = await User.aggregate([
    {
      $facet: {
        ...
        newInPeriod: [{ $match: { createdAt: { $gte: start, $lte: end } } }, { $count: 'count' }],
        byRole: [{ $group: { _id: '$role', count: { $sum: 1 } } }],
        byAuthProvider: [{ $group: { _id: '$authProvider', count: { $sum: 1 } } }],
      },
    },
  ]);
  ...
  const report = {
    ...
    summary: {
      ...
      byRole:
        summaryData?.byRole?.map((r: any) => ({
          role: r._id || 'unknown',
          count: r.count,
        })) || [],
      byAuthProvider:
        summaryData?.byAuthProvider?.map((p: any) => ({
    ...
    engagementDistribution:
      engagementData?.engagement?.map((e: any) => ({
