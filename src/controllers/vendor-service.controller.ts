...
export const updateVendorService = asyncHandler(async (req: Request, res: Response) => {
  ...
  const vendorService = await VendorService.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true,
  });
  if (!vendorService) {
...
export const searchVendorServices = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (city || state || country) {
    ...
    if (filter.vendorId) {
      ...
      filter.vendorId = {
        $in: vendorIdsFromLocation.filter((id) => id.toString() === filter.vendorId.toString()),
      };
    } else {
