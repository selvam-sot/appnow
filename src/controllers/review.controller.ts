...
export const createReview = asyncHandler(async (req: Request, res: Response) => {
  ...
  const appointment = await Appointment.findById(appointmentId).populate('vendorServiceId');
  ...
  const vendorServiceIdValue =
    (appointment.vendorServiceId as any)?._id || appointment.vendorServiceId;
...
export const getVendorServiceReviews = asyncHandler(async (req: Request, res: Response) => {
  ...
  const ratingStats = await Review.aggregate([
    {
      $match: {
        vendorServiceId: require('mongoose').Types.ObjectId.createFromHexString(vendorServiceId),
        status: 'approved',
      },
    },
    {
