...
export const getTodayAppointments = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.status(200).json({
    ...
    data: appointments.map((apt) => ({
      id: apt._id,
      customerName:
        `${(apt.customerId as any)?.firstName || ''} ${(apt.customerId as any)?.lastName || ''}`.trim(),
...
export const updateProfile = asyncHandler(async (req: Request, res: Response) => {
  ...
  const vendor = await Vendor.findByIdAndUpdate(vendorId, filteredUpdates, { new: true });
...
export const getServiceSlots = asyncHandler(async (req: Request, res: Response) => {
  ...
  const slotDocs = await VendorServiceSlot.find(query).sort({ year: 1 }).lean();
...
export const getAnalytics = asyncHandler(async (req: Request, res: Response) => {
  ...
  const completionRate =
    currentBookings > 0 ? Math.round((completedCurrent / currentBookings) * 100) : 0;
