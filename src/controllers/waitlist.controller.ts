  ...
  try {
    ...
    for (const entry of entries) {
      try {
        ...
        logger.info(
          `Waitlist notification sent to user ${entry.clerkId} for service ${vendorServiceId} on ${dateStr}`,
        );
    ...
    logger.info(
      `Notified ${entries.length} waitlisted user(s) for service ${vendorServiceId} on ${dateStr}`,
    );
  } catch (error: any) {
