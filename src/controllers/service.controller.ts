...
export const getServiceList = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const limit =
      req.body.limit && Number.isInteger(Number(req.body.limit))
    ...
    const serviceIdsFromVS = [
      ...new Set(matchingVendorServices.map((vs) => (vs as any).serviceId.toString())),
    ];

    // Build OR conditions: match service name, or parent category/subcategory, or linked vendor service
    const orConditions: any[] = [{ name: nameRegex }];
