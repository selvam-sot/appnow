...
export const getVendorServiceSlotById = asyncHandler(async (req: Request, res: Response) => {
  const vendorServiceSlot = await VendorServiceSlot.findById(req.params.id).populate(
    'vendorServiceId',
  );
...
export const updateVendorServiceSlot = asyncHandler(async (req: Request, res: Response) => {
  ...
  const vendorServiceSlot = await VendorServiceSlot.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true,
  });
  if (!vendorServiceSlot) {
