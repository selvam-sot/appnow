...
} from '../models/scheduled-notification.model';
...
export async function scheduleAppointmentReminders(appointment: AppointmentDetails): Promise<void> {
  try {
    ...
    if (['cancelled', 'completed', 'failed', 'missed'].includes(appointment.status)) {
      logger.info(
        `[NotificationScheduler] Skipping reminders for ${appointment.status} appointment ${appointmentId}`,
      );
      return;
    }
    ...
    if (notificationsToCreate.length > 0) {
      ...
      logger.info(
        `[NotificationScheduler] Scheduled ${notificationsToCreate.length} reminders for appointment ${appointmentId}`,
      );
    } else {
      logger.info(
        `[NotificationScheduler] No reminders to schedule for appointment ${appointmentId} (all already scheduled or past)`,
      );
    }
  } catch (error: any) {
...
export async function cancelAppointmentReminders(appointmentId: string): Promise<number> {
  try {
    ...
    if (result.modifiedCount > 0) {
      logger.info(
        `[NotificationScheduler] Cancelled ${result.modifiedCount} reminders for appointment ${appointmentId}`,
      );
    }
...
export async function processScheduledNotifications(): Promise<void> {
  try {
    ...
    for (const notification of dueNotifications) {
      try {
        ...
        if (result.success) {
          ...
          logger.info(
            `[NotificationScheduler] Sent ${notification.type} to customer ${customer.firstName || customer.email || customer._id}`,
          );
        } else {
        ...
        logger.error(
          `[NotificationScheduler] Error processing notification ${notification._id}: ${error.message}`,
        );
        await ScheduledNotification.findByIdAndUpdate(notification._id, {
          $inc: { retryCount: 1 },
    ...
    logger.error(
      `[NotificationScheduler] Error in processScheduledNotifications: ${error.message}`,
    );
  }
}
...
export async function scheduleDailyReminders(): Promise<void> {
  try {
    ...
    logger.info(
      `[NotificationScheduler] Daily scheduler completed. Checked ${appointments.length} appointments, scheduled reminders for ${scheduledCount}`,
    );
  } catch (error: any) {
...
export function startNotificationScheduler(): void {
  ...
  logger.info(
    '[NotificationScheduler] Notification scheduler started (processor: every 1 min, daily: 12:01 AM)',
  );
}
...
export async function rescheduleAppointmentReminders(appointmentId: string): Promise<void> {
  try {
    ...
    if (!appointment) {
      logger.warn(
        `[NotificationScheduler] Appointment ${appointmentId} not found for rescheduling`,
      );
      return;
    }
...
export async function syncTodaySchedule(): Promise<void> {
  try {
    ...
    if (scheduledCount > 0) {
      logger.info(
        `[NotificationScheduler] Synced today's schedule. Scheduled reminders for ${scheduledCount} appointments`,
      );
    }
