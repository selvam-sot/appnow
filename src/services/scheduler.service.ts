...
async function send24HourReminders(now: Date): Promise<void> {
  try {
    ...
    const appointments = await findAppointmentsForReminder(
      startWindow,
      endWindow,
      'reminder24hSentAt',
    );
...
async function sendReminderForAppointment(
  ...
  try {
    ...
    if (result.success) {
      logger.info(
        `${reminderType} reminder sent for appointment ${appointment._id} to customer ${customer.firstName || customer.email || customerId}`,
      );
      return true;
    } else {
      logger.warn(
        `Failed to send ${reminderType} reminder for appointment ${appointment._id}: ${result.error}`,
      );
      return false;
    }
  } catch (error: any) {
...
export function startReminderScheduler(): void {
  ...
  logger.info(
    'Appointment scheduler started (reminders: every 5 min, auto-complete: every 15 min)',
  );
}
...
export async function sendManualReminder(
  ...
  try {
    ...
    if (sent) {
      ...
      return {
        success: false,
        message: 'Failed to send reminder - check if customer has push token',
      };
    }
  } catch (error: any) {
